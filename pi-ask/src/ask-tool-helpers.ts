...
export function renderAskToolCall(args: unknown, theme: ToolTheme) {
	...
	const labels = Array.isArray(params.questions)
		? params.questions
				...
				.join(", ")
		: "";
	let text = `${theme.fg("warning", "?")} ${theme.fg("toolTitle", theme.bold("Ask user"))} `;
...
export function renderAskToolResult(
	...
	if (details.error) {
		return new Text(
			`${theme.fg("warning", "✗")} ${theme.fg("toolTitle", theme.bold("Ask user"))} ${theme.fg("warning", "Invalid input")}`,
			0,
			0
		);
	}
	if (details.cancelled) {
		return new Text(
			`${theme.fg("warning", "✗")} ${theme.fg("toolTitle", theme.bold("Ask user"))} ${theme.fg("warning", "Cancelled")}`,
			0,
			0
		);
	}
	return new Text(
		`${theme.fg("success", "✓")} ${theme.fg("toolTitle", theme.bold("Ask user"))}\n${renderResultText(details)}`,
		0,
		0
	);
}
